...
const PreferencesMenu = () => {
    ...
    const handleToggleCacheCSV = async () => {
        if (cacheProcessedCSV && cachedCSV) {
            if (
                window.confirm(
                    'This will delete your cached processed data. Continue?'
                )
            ) {
                removeCachedCSV();
    ...
    const handleToggleCacheApi = () => {
        if (cacheApiData && Object.keys(cachedSongData).length > 0) {
            if (
                window.confirm(
                    'This will delete your cached API data. Continue?'
                )
            ) {
                removeCachedSongData();
    ...
    const handleReset = async () => {
        if (
            window.confirm(
                'This will delete all data and reset the application. Continue?'
            )
        ) {
            await clearAllData();
    ...
    return (
        <div className="preferences-menu">
            <div className="preferences-section">
                ...
                <label className="preferences-option">
                    <input
                        ...
                        onChange={(e) =>
                            setShowPrivacyDisclaimer(e.target.checked)
                        }
                    />
            ...
            <div className="preferences-section">
                ...
                <button
                    ...
                    onClick={() => {
                        if (
                            googleApiKey &&
                            window.confirm('Delete your Google API key?')
                        ) {
                            removeGoogleApiKey();
                        }
                    }}
                ...
                <button className="preferences-button-danger" disabled={true}>
            ...
            <div className="preferences-section">
                ...
                <button className="preferences-button" disabled={true}>
