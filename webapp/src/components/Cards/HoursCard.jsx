...
const HoursCard = ({ data }) => {
  ...
  const chartConfig = useMemo(() => {
    const labels = data?.labels?.map(hour => hour.split(':')[0]) || Array.from({ length: 24 }, (_, i) => String(i).padStart(2, '0'));
    const filteredData = data?.datasets?.[0]?.data || new Array(24).fill(0);
    const otherData = data?.datasets?.[1]?.data || new Array(24).fill(0);
    const totalData = filteredData.map((val, i) => val + (otherData[i] || 0));
    ...
    if (showStacked) {
      ...
      const displayData = isFiltered && isZoomed ? filteredData : totalData;
      datasets = [{
        label: 'Plays',
        data: displayData,
        backgroundColor: chartColors.accent,
