...
const TimelineCard = ({ data }) => {
  const { filters, updateFilter, dateRange, hasActiveFilters, clearAllFilters } = useApp();
  ...
  const chartData = useMemo(() => {
    ...
    return data.labels.map((date, index) => ({
      ...
      count: (data.datasets[0]?.data[index] || 0) + (data.datasets[1]?.data[index] || 0),
  ...
  const displayData = useMemo(() => {
    ...
    return chartData.filter(d => d.timestamp >= filters.dateRange.start && d.timestamp <= filters.dateRange.end);
  ...
  useEffect(() => {
    ...
    const config = {
      ...
      options: {
        ...
        scales: {
          x: { stacked: true, ticks: { color: chartColors.text.secondary, maxRotation: 0, autoSkip: true, autoSkipPadding: 30 } },
          y: { stacked: true, ticks: { color: chartColors.text.secondary }, grid: { color: commonChartStyles.grid.stroke } }
        },
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              title: (items) => format(parseISO(items[0].label), 'MMM d, yyyy'),
              label: (ctx) => `${ctx.dataset.label}: ${ctx.raw}`,
              footer: (items) => showStacked ? `Total: ${displayData[items[0].dataIndex].count}` : ''
            }
          },
  ...
  useEffect(() => {
    ...
    if (showStacked) {
      datasets = [
        { label: 'Filtered', data: displayData.map(d => d.filtered), backgroundColor: chartColors.accent },
        { label: 'Other', data: displayData.map(d => d.other), backgroundColor: 'rgba(100, 100, 100, 0.5)' }
      ];
    } else {
      const dataKey = isFiltered && isZoomed ? 'filtered' : 'count';
      datasets = [{ label: 'Plays', data: displayData.map(d => d[dataKey]), backgroundColor: chartColors.accent }];
    }
  ...
  useEffect(() => {
    if (chartInstance.current) {
      chartInstance.current.options.plugins.selectionPlugin.dragStart = dragStartIndex;
      chartInstance.current.options.plugins.selectionPlugin.dragEnd = dragEndIndex;
  ...
  useEffect(() => {
    ...
    const getIndexFromEvent = (e) => {
      ...
      const elements = chart.getElementsAtEventForMode(e, 'index', { intersect: false });
    ...
    const handleMouseUp = () => {
      if (isDragging && dragStartIndex !== null && dragEndIndex !== null && dragStartIndex !== dragEndIndex) {
        const start = displayData[Math.min(dragStartIndex, dragEndIndex)].timestamp;
        const end = displayData[Math.max(dragStartIndex, dragEndIndex)].timestamp;
    ...
    <div className="card full-width">
      <div className="card-header">
        ...
        <div className="card-header-controls">
          {isFiltered && (
            <>
              <button id="zoom-toggle-btn" title={isZoomed ? "Show full range" : "Zoom to selection"} onClick={() => setIsZoomed(!isZoomed)}>
                {isZoomed ? '⤆' : '⤢'}
              </button>
              <button id="clear-filter-btn" title="Clear all filters" onClick={clearAllFilters}>
                ↺
              </button>
            </>
          )}
      ...
      <div className="card-content">
        ...
        <div className="chart-wrapper" style={{ userSelect: 'none', height: '350px' }}>
          <canvas ref={chartRef} />
        </div>
      </div>
    </div>
