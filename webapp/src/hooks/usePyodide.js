...
function usePyodide() {
  ...
  useEffect(() => {
    const initPyodide = async () => {
      try {
        ...
        const response = await fetch(import.meta.env.BASE_URL + 'ytmwrapped.py');
  ...
  const runPythonFunction = (functionName, ...args) => {
    ...
    if (!func) {
      ...
      console.error('Available functions:', Array.from(pyodide.globals.keys()));
      throw new Error(`Python function '${functionName}' is not available`);
    }
