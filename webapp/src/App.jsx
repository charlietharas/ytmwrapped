...
function AppContent() {
    ...
    useEffect(() => {
        ...
        const updateCardsWithFiltersHandler = async () => {
            try {
                ...
                filters.songs.forEach((song) =>
                    activeFilters.push({ type: 'song', value: song })
                );
                filters.artists.forEach((artist) =>
                    activeFilters.push({ type: 'artist', value: artist })
                );
                filters.hours.forEach((hour) =>
                    activeFilters.push({ type: 'hour', value: hour })
                );
                filters.daysOfWeek.forEach((day) =>
                    activeFilters.push({ type: 'dayOfWeek', value: day })
                );
    ...
    const handleFilesSelected = async (files, useCache = false) => {
        if (useCache && cachedCSV) {
            ...
            if (!pyodide) {
                alert(
                    'Python environment is still loading. Please wait a moment and try again.'
                );
                return;
            }
